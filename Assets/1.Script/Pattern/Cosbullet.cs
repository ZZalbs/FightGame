...
public class CosBullet : BulletType
{
    ...
    IEnumerator SmoothMove()
    {
        ...
        for (int i = 0; i <= 20; i++)
        {
            gameObject.transform.position = Vector2.Lerp(curPos, new Vector2(curPos.x,  Mathf.Cos(xChange)+yline), 0.05f * i);
            // y = x * sin(x)
    ...
    IEnumerator ChangetoCos()
    {
        ...
        for (int i = 0; i <= 20; i++)
        {
            gameObject.transform.position = Vector2.Lerp(curPos, new Vector2(curPos.x, (-1)*Mathf.Cos(xChange)+yline), 0.05f * i);
            // y = cos(x)
            yield return new WaitForSeconds(0.05f);
        }
        //curPos = new Vector2(xChange, gameObject.transform.position.y);
